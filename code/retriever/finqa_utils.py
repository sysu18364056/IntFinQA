...
class MathQAExample(
        collections.namedtuple(
            "MathQAExample",
            "filename_id question intent all_positive \
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.intent_input_ids=intent_input_ids
        self.input_mask=intent_input_mask
        self.intent_segment_ids=intent_segment_ids
        self.label = label
...
def wrap_single_pair(tokenizer, question, intent, context, label, max_seq_length,  
                    cls_token, sep_token):
    '''
    single pair of question, context, label feature
    '''
    intent=intent.split('/',1)
    primary_intent=intent[0]   
    #print(primary_intent)


    deep_intent=intent[1]

    #question_tokens = tokenize(tokenizer, question+primary_intent)   
    
    question_tokens = tokenize(tokenizer, question)
   
    intent_tokens=tokenize(tokenizer,deep_intent)
    intent_tokens=[cls_token]+intent_tokens+[sep_token]
    intent_segment_ids=[0]*len(intent_tokens)
    intent_input_ids=tokenizer.convert_tokens_to_ids(intent_tokens)
    intent_input_mask=[1]*len(intent_input_ids)
    intent_padding = [0] * (max_seq_length - len(intent_input_ids))
    intent_input_ids.extend(intent_padding)
    intent_input_mask.extend(intent_padding)
    intent_segment_ids.extend(intent_padding)
    ...
    this_input_feature = {
        ...
        "intent_input_mask":intent_input_mask,
        "intent_segment_ids":intent_segment_ids,
        "label": label
    }
    ...
    if is_training:
        for gold_ind in example.all_positive:
            ...
            this_input_feature = wrap_single_pair(
                tokenizer, question, intent, this_gold_sent, 1, max_seq_length,
                cls_token, sep_token)
        ...
        for i in range(len(all_text)):
            if i not in pos_text_ids:
                ...
                this_input_feature = wrap_single_pair(
                    tokenizer, example.question,example.intent, this_text, 0, max_seq_length,
                    cls_token, sep_token)
                this_input_feature["filename_id"] = example.filename_id
        ...
        for this_table_id in range(len(example.table)):
            if this_table_id not in pos_table_ids:
                ...
                this_input_feature = wrap_single_pair(
                    tokenizer, example.question, example.intent, this_table_line, 0, max_seq_length,
                    cls_token, sep_token)
                this_input_feature["filename_id"] = example.filename_id
        ...
        for i in range(len(all_text)):
            this_text = all_text[i]
            this_input_feature = wrap_single_pair(
                tokenizer, example.question, example.intent, this_text, -1, max_seq_length,
                cls_token, sep_token)
            this_input_feature["filename_id"] = example.filename_id
            this_input_feature["ind"] = "text_" + str(i)
            features_neg.append(this_input_feature)
        ...
        for this_table_id in range(len(example.table)):
            ...
            this_table_line = table_row_to_text(example.table[0], example.table[this_table_id])
            this_input_feature = wrap_single_pair(
                tokenizer, example.question, example.intent, this_table_line, -1, max_seq_length,
                cls_token, sep_token)
            this_input_feature["filename_id"] = example.filename_id
            this_input_feature["ind"] = "table_" + str(this_table_id)
            features_neg.append(this_input_feature)
