...
def train():
    ...
    for _ in range(conf.epoch):
        ...
        for x in train_iterator:
            ...
            intent_input_ids=torch.tensor(x['intent_input_ids']).to(conf.device)
            intent_input_mask=torch.tensor(x['intent_input_mask']).to(conf.device)
            intent_segment_ids= torch.tensor(x['intent_segment_ids']).to(conf.device)
            ...
            this_logits = model(True, input_ids, input_mask,
                                segment_ids, intent_input_ids,intent_input_mask,intent_segment_ids,option_mask, program_ids, program_mask, device=conf.device)
...
def evaluate(data_ori, data, model, ksave_dir, mode='valid'):
    ...
    with torch.no_grad():
        for x in tqdm(data_iterator):
            ...
            intent_input_ids=x['intent_input_ids']
            intent_input_mask=x['intent_input_mask']
            intent_segment_ids=x['intent_segment_ids']

            ori_len = len(input_ids)
            for each_item in [input_ids, input_mask, segment_ids, program_ids, program_mask, option_mask,intent_input_ids,intent_input_mask,intent_segment_ids]:
            ...
            intent_input_ids = torch.tensor(intent_input_ids).to(conf.device)
            intent_input_mask = torch.tensor(intent_input_mask).to(conf.device)
            intent_segment_ids = torch.tensor(intent_segment_ids).to(conf.device)
