...
class Bert_model(nn.Module):
    ...
    def forward(self, is_training, input_ids, input_mask, segment_ids, intent_input_ids,intent_input_mask,intent_segment_ids, option_mask, program_ids, program_mask, device):
        ...
        if bert_sequence_output[:, 0, :].size()!=bert_pool_output.size():
       
            bert_pool_output=bert_pool_output.resize_( bert_sequence_output[:, 0, :].size())

        bert_pooled_output = bert_sequence_output[:, 0, :]+bert_pool_output 
